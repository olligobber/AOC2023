...
getTypeWithWild :: Hand -> HandType 
getTypeWithWild (Hand cards) = maximum $ do 
	replaceJacksWith <- allCards
	let newCards = fmap (\c -> if c == Jack then replaceJacksWith else c) cards 
	pure $ getType $ Hand newCards
...
scoreGames gs = sum $ zipWith scoreGame [1..] $ sortBy (comparing valGame) gs where 
	...
	valGame (game, _) = (getTypeWithWild game, game)
